...
public class SimulationDataManager
{
    ...
    // Core death age data
    private List<(float age, CreatureSim.DeathCause cause)> _deathAges;
    ...
    public SimulationDataManager(string basePath, DataMode mode, float saveInterval = 5f)
    {
        ...
        {
            _deathAges = new List<(float, CreatureSim.DeathCause)>();
    ...
    public void RecordDeath(float age, CreatureSim.DeathCause cause)
    {
    ...
    public void RecordDataIfItIsTime(float currentSimTime)
    {
        ...
        if (currentSimTime - _lastSaveTime >= _saveInterval)
        {
            ...
            foreach (var cause in Enum.GetValues(typeof(CreatureSim.DeathCause)))
            {
                var causeHistogram = BarDataUtilities.MakeHistogram(
                    _deathAges
                        .Where(x => x.Item2 == (CreatureSim.DeathCause)cause)
                        .Select(x => x.Item1)
                ).ToArray();
